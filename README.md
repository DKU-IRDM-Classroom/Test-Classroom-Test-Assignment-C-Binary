# Test Classroom / Test Assignment C Binary

## 문제

입력으로 정수 $a$, $b$가 차례로 들어온다. 이 둘의 합 $a + b$를 계산하여 **개행 없이**, **null 문자 없이**, 문자열로 출력하는 프로그램을 작성하시오.
...
입력 파일을 입력으로 생각하고, 출력용 **바이너리** 파일 `{t}.out`을 출력으로 생각한다.
